...
def applyTags(row):
    ...
    if n % 250 == 0:
        ...
        delta = now - t
        avg = delta / n
        avgMicroseconds = int((avg * 1e6))
        projected = (NUM_TAGGED - n) * avg
        percent = (n / NUM_TAGGED) * 100
        # AI disclosure - Literally just the '\r' and 'flush=True' parts were generated by AI. I kid you not. Nothing else. If that was actually worth some kind of marks I'm flabbergasted.
        print(f"\r    Tagged {n} boards ({percent:.2f}%) ({delta:.1f}s; avg {avgMicroseconds}μs; projected {projected:.1f}s)", end="", flush=True)
...
print("Loading df...")
df = pd.read_csv(path, usecols=["id", "puzzle", "clues", "difficulty", "solution"], nrows=(None if LOAD_MAX <= 0 else LOAD_MAX)).rename(columns={"id": "PuzzleID", "puzzle": "SerialisedBoard", "solution": "SerialisedSolution"})
...
n = 0
df['SerialisedSolution'] = df['SerialisedSolution'].apply(lambda x:convertPuzzleString(str(x)))
print()
