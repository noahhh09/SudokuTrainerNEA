...
class NakedGroup(Technique):
    displayName = "Naked Group"
    description = "A technique where n cells contain only n mutual candidates between them, allowing candidates to be ruled out in other cells."
    degree: int
    ...
    def findAvailable(cls, state: BoardState) -> list[Technique]:
        ...
        for unit in units:
            ...
            nakedGroups = _findNakedGroups(cells, cls.degree)
            for positions, union in nakedGroups:
                moves: list[Move] = []
                ...
                if len(moves) > 0:
                    tech = cls(moves, unit, set(positions), union)
                    found.append(tech)
...
class NakedPair(NakedGroup):
    displayName = "Naked Pair"
    description = "A technique where two cells exclusively have the same two candidates remaining."
    degree = 2

class NakedTriple(NakedGroup):
    displayName = "Naked Triple"
    description = "A technique where 3 cells contain only 3 mutual candidates between them, allowing candidates to be ruled out in other cells."
    degree = 3
